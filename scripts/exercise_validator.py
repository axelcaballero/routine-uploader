...
Validates exercise template IDs against the authoritative source (exercise_mappings.md)
...
class ExerciseValidator:
    """Validates exercise IDs against exercise_mappings.md mappings"""
    
    def __init__(self, instructions_path: str = "exercise_mappings.md"):
        """
        Initialize validator with instructions file.
        
        Args:
            instructions_path: Path to exercise_mappings.md file
        """
    ...
    def _parse_instructions(self) -> Dict[str, Dict[str, str]]:
        """
        Parse exercise_mappings.md to extract exercise mappings.
    ...
    def validate_routine(self, routine_data: Dict, verbose: bool = False, interactive: bool = False) -> Tuple[bool, List[str]]:
        """
        Validate all exercises in a routine against exercise_mappings.md.
        ...
        print(f"\n🔍 Validating {len(exercises)} exercises against exercise_mappings.md...\n")
        ...
        for i, exercise in enumerate(exercises, 1):
            ...
            if found_in_map:
                ...
                status = "⚠"
                message = f"{i}. {ex_id} - NOT FOUND IN exercise_mappings.md! Notes: {notes[:50]}"
    ...
    def _handle_missing_exercise(self, missing_info: Dict) -> None:
        ...
        and adding it to exercise_mappings.md
    ...
    def _add_exercise_to_instructions(self, spanish_name: str, english_name: str, template_id: str, category: str) -> None:
        ...
        try:
            ...
            if not re.search(category_pattern, content, re.MULTILINE):
                print(f"   ⚠️  Category '{category}' not found in exercise_mappings.md")
                return
            ...
            print(f"   ❌ Error adding exercise to exercise_mappings.md: {e}")
    ...
    def print_available_exercises(self) -> None:
        """Print all available exercises from exercise_mappings.md"""
        print("\n📚 Available Exercises from exercise_mappings.md:\n")
        print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate routine exercise IDs against exercise_mappings.md"
    )
    parser.add_argument(
        "routine_file",
        nargs="?",
    ...
    parser.add_argument(
        "-m", "--mappings", "--instructions",
        dest="mappings_file",
        default="exercise_mappings.md",
        help="Path to exercise_mappings.md (default: exercise_mappings.md)"
    )
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
    ...
    parser.add_argument(
        "-i", "--interactive",
        action="store_true",
        help="Prompt for missing exercises and add them to exercise_mappings.md"
    )
    parser.add_argument(
        "--list",
        action="store_true",
        help="List all available exercises from exercise_mappings.md"
    )
    
    args = parser.parse_args()
    
    validator = ExerciseValidator(args.mappings_file)
