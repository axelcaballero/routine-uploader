#!/usr/bin/env python3
"""
Bench Press Progression Tracker
Fetches bench press data from Hevy API and visualizes progression over time.
"""
...
def find_bench_press_template(client: HevyAPIClient) -> Tuple[str, str]:
    """
    Get the bench press exercise template.
    Barbell Bench Press ID: 79D0BB3A
...
def fetch_bench_press_history(client: HevyAPIClient, exercise_template_id: str) -> List[Dict[str, Any]]:
    """
    Fetch bench press workout history.
...
def chart_progression(workouts: List[Dict[str, Any]], year: int, exercise_name: str):
    """Create visualization of bench press progression."""
    ...
    ax1.plot(max_by_date['date'], max_by_date['weight'], 
             marker='o', linewidth=2, markersize=6, color='#3498db', label='Max weight per session')
    ...
    ax2.plot(estimated_1rm['date'], estimated_1rm['estimated_1rm'], 
             marker='s', linewidth=2, markersize=6, color='#e74c3c', label='Estimated 1RM (Epley)')
    ...
    # Save and show
    filename = f'bench_press_progression_{year}.png'
...
def main():
    ...
    try:
        ...
        # Find bench press template
        print("\nSearching for bench press exercise...")
        template_id, exercise_name = find_bench_press_template(client)
        print(f"Found: {exercise_name} (ID: {template_id})")
        
        # Fetch history
        print("\nFetching exercise history...")
        all_workouts = fetch_bench_press_history(client, template_id)
        ...
        # Save data to JSON
        data_file = 'bench_press_data_2025.json'
