#!/usr/bin/env python3
"""
Barbell Squat Progression Tracker
Fetches squat data from Hevy API and visualizes progression over time.
"""
...
def get_squat_exercise(client: HevyAPIClient) -> Tuple[str, str]:
    """
    Get the squat exercise template.
    Barbell Squat ID: D04AC939
...
def fetch_squat_history(client: HevyAPIClient, exercise_template_id: str) -> List[Dict[str, Any]]:
    """
    Fetch squat workout history.
...
def print_summary(workouts: List[Dict[str, Any]], year: int):
    ...
    print(f"\n=== Barbell Squat Progression - {year} ===")
...
def chart_progression(workouts: List[Dict[str, Any]], year: int, exercise_name: str):
    """Create visualization of squat progression."""
    ...
    ax1.plot(max_by_date['date'], max_by_date['weight'], 
             marker='o', linewidth=2, markersize=6, color='#2ecc71', label='Max weight per session')
    ...
    ax2.plot(estimated_1rm['date'], estimated_1rm['estimated_1rm'], 
             marker='s', linewidth=2, markersize=6, color='#f39c12', label='Estimated 1RM (Epley)')
    ...
    # Save and show
    filename = f'squat_progression_{year}.png'
...
def main():
    ...
    try:
        ...
        # Get squat template
        print("\nSearching for squat exercise...")
        template_id, exercise_name = get_squat_exercise(client)
        print(f"Found: {exercise_name} (ID: {template_id})")
        
        # Fetch history
        print("\nFetching exercise history...")
        all_workouts = fetch_squat_history(client, template_id)
        ...
        # Save data to JSON
        data_file = 'squat_data_2025.json'
